...
$(document).ready(function () {
    ...
    $("#myWallet").on("change", function (e){
        e.preventDefault();
        //debugger
        const wallet_balance = $( this ).data( 'balance' );
        const price = $( this ).data( 'price' );
        let new_price = price;
        let dr_wallet_amount = 0;
        if ( $( this ).prop( "checked" ) === true ) {
            if ( price > wallet_balance ) {
                new_price =  price - wallet_balance;
            } else {
                new_price = wallet_balance - price;
                dr_wallet_amount = wallet_balance - new_price;
                new_price = dr_wallet_amount == price ? 0 : price; 
            }
            bookingMedicine.append("wallet_balance", dr_wallet_amount);
        }
        $( document ).find( ".finalPrice,.subtotal" ).text( new_price );
    });
});
...
function wallet()
{
    var ref = Math.floor(Math.random() * (999999 - 111111)) + 999999;
    bookingMedicine.append("payment_token", ref);
    bookingMedicine.append("payment_type", "wallet");
    bookingMedicine.append("payment_status", 1);
